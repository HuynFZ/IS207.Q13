      ...
      <main class="chat-main" v-if="activeConversation">
        <header class="chat-header">
          <div class="header-left">
            ...
            <div>
              ...
              <span>Hoạt động 4 ngày trước</span>
            </div>
          </div>
        ...
        <div class="message-area" ref="messageAreaRef">
          <div class="product-info-card">
            <img src="/avatar.jpg" alt="Sản phẩm" class="product-image">
            <div class="product-details">
              <span>Tai nghe Sony</span>
              <span>2.000.000 đ</span>
            </div>
          </div>
          <div class="date-divider"><span>Chủ nhật, 05/10/2025</span></div>
          <div 
        ...
        <footer class="message-input-area">
          <div class="quick-replies">
            <button @click="sendQuickReply('Tai nghe này còn không?')">Tai nghe này còn không?</button>
            <button @click="sendQuickReply('Bạn có ship hàng không?')">Bạn có ship hàng không?</button>
          </div>
          <div class="input-row">
            <button class="icon-btn"><font-awesome-icon icon="camera" /></button>
            <button class="icon-btn"><font-awesome-icon icon="map-marker-alt" /></button>
...
// BỔ SUNG: "Theo dõi" trạng thái đăng nhập
watch(isLoggedIn, (isNowLoggedIn) => {
  if (isNowLoggedIn === false) {
    // Nếu người dùng bị đăng xuất (ví dụ: token hết hạn hoặc bấm nút logout)
    // Tự động đá về trang chủ
    router.push('/');
  }
});

// "Cơ sở dữ liệu" tin nhắn (Giữ nguyên)
const allMessages = ref({
  1: [
    { id: 101, text: 'Sản phẩm này đã qua sử dụng chưa?', type: 'sent', timestamp: '10:00' },
  ],
  2: [
    { id: 201, text: 'Bạn có ship không?', type: 'received', timestamp: '10:01' },
    { id: 202, text: 'Có bạn nhé, bạn ở đâu?', type: 'sent', timestamp: '10:02' },
  ],
  3: [
    { id: 301, text: 'Bạn sản phẩm này đã qua sử dụng?', type: 'received', timestamp: '10:03' },
  ]
});

// Lấy thông tin của hội thoại đang active (Giữ nguyên)
const activeConversation = computed(() => {
  return conversations.value.find(c => c.id === activeConversationId.value);
});

// Lấy tin nhắn của hội thoại đang active
const activeMessages = computed(() => {
  if (!activeConversationId.value) return [];
  return allMessages.value[activeConversationId.value] || [];
});

// BỔ SUNG: Lọc danh sách hội thoại (Sidebar)
const filteredConversations = computed(() => {
  return conversations.value.filter(convo => {
    // 1. Lọc theo Tab (Tất cả / Chưa đọc)
...
onMounted(() => {
  ...
  conversations.value = [
    { id: 1, name: 'Cửa Hàng Giá Rẻ', preview: 'Sản phẩm này đã qua sử dụng chưa?', time: 'Vài giây trước', avatar: '/avatar.jpg', unread: false },
    { id: 2, name: 'Điện Thoại Vui', preview: 'Bạn có ship không?', time: '5 phút trước', avatar: '/avatar.jpg', unread: false },
    { id: 3, name: 'Smartphone Rẻ', preview: 'Bạn sản phẩm này đã qua sử dụng?', time: '1 tiếng trước', avatar: '/avatar.jpg', unread: true },
  ];
...
// SỬA ĐỔI 6: Sửa sendMessage để nhận tham số
const sendMessage = (textToSend = null) => {
  // Lấy text từ (1) nút gợi ý, hoặc (2) ô input
  const text = textToSend || newMessage.value;

  if (text.trim() === '') return;

  // (Demo: Thêm tin nhắn vào mảng local)
  const newMsg = {
    id: Date.now(),
    text: text, // Dùng biến 'text'
    type: 'sent',
  ...
  if (allMessages.value[activeConversationId.value]) {
    ...
    allMessages.value[activeConversationId.value] = [newMsg];
  }

  // Cập nhật preview (demo)
  const convo = conversations.value.find(c => c.id === activeConversationId.value);
  if (convo) convo.preview = text; // Dùng biến 'text'

  newMessage.value = ''; // Xóa ô input
  scrollToBottom();
};

// SỬA ĐỔI 7: Thêm hàm sendQuickReply
const sendQuickReply = (text) => {
  sendMessage(text); // Gọi hàm sendMessage với nội dung gợi ý
};
...
.chat-page-container.empty-state {
  display: flex;
...
/* (Đây là CSS cho .search-wrapper mà bạn đã dán) */
.search-wrapper {
  position: relative;
  margin-bottom: 1rem;
}
...
/* BỔ SUNG: CSS cho file SVG */
.empty-svg {
  max-width: 250px;
  margin-bottom: 1rem;
  color: #ccc; /* (Một số file SVG có thể nhận màu chữ) */
}
</style>